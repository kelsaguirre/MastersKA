...
%focus on just log not lin
    ...
    %disp(all_IDs(iS,:));
    cd(root_folder)
    save('partial_slot_opt_bays_dyn_baseline', 'best_set');
end

mean_bs=mean(best_set(1:n_subjects,:))
std_bs=std(best_set(1:n_subjects,:))

%can add an if to call it baseline

save('slot_opt_bays_dyn_baseline', 'best_set', 'all_cond3', 'all_t_data', 'all_data', 'all_IDs');
