...
for iS = 1:n_subjects
    ...
    toc
    %added data here
    %load(internalCellArray(subj{iS}));
    %IDs{iS}=subj{iS}(9:10);
    load(subj{iS});
    IDs{iS}=subj{iS}(9:10);
    ...
    %data=data_folder;
    [alphalin,fvalin] = fminsearch(@(pars) RLmodel_lin(pars,data), 0.2);
    [alphalog,fvalog] = fminsearch(@(pars) RLmodel_log(pars,data), 0.2);
    ...
    disp(best_set(iS,:));
    cd(root_folder)
    save('partial_slot_opt_RL_baseline','best_set');
end

mean_bs=mean(best_set(1:n_subjects,:))
std_bs=std(best_set(1:n_subjects,:))

save('slot_opt_RL_baseline.mat', 'best_set', 'all_cond3', 'all_t_data', 'all_data', 'all_IDs');
