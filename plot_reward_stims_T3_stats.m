...
% Load treatment data
load('slot_T3_data.mat', 'Lat_all_data_clean', 'Off_all_data_clean', 'Thera_all_data_clean'); 

% Calculate mean and standard error for baseline data
[baseline_mean, baseline_means] = calculate_mean_reward(baseline_all_data);
baseline_se = calculate_standard_error_reward(baseline_all_data);
...
[off_mean, off_means] = calculate_mean_reward(Off_all_data_clean);
[thera_mean, thera_means] = calculate_mean_reward(Thera_all_data_clean);
[lat_mean, lat_means] = calculate_mean_reward(Lat_all_data_clean);

% Calculate standard errors for each condition
off_se = calculate_standard_error_reward(Off_all_data_clean);
thera_se = calculate_standard_error_reward(Thera_all_data_clean);
lat_se = calculate_standard_error_reward(Lat_all_data_clean);
...
ylabel('Mean Reward/Subject');
ylim([0 6000]);
title_text = 'Mean Reward Pre-DBS and T3'; % Define title text
title(title_text);
text(0.5, 5500, 'Pre-DBS N = 11', 'FontSize', 14);
text(3.5, 5500, 'T3 N = 9', 'FontSize', 14);
...
maxHeight = max(4700); % Adjust as needed based on your plot's ylim
...
for i = 1:length(comparisons)
    ...
    lineHeight = maxHeight + (i-1) * 240;
    
    % Draw the line
    line(comparisons(i).groups, [lineHeight, lineHeight], 'Color', 'k', 'LineWidth', 1.5);
    
    % Add text (significance level)
    text(mean(comparisons(i).groups), lineHeight + 120, sigLevel, 'HorizontalAlignment', 'center', 'FontSize', 11);
end

% Helper function to calculate mean reward from the nested cell structure
function [mean_reward, subject_means] = calculate_mean_reward(data)
    total_sum = 0;
    total_count = 0;
    ...
    for i = 1:numel(data)
        ...
        for j = 1:numel(data{i})
            matrix = data{i}{j};
            if ~isempty(matrix)
                subject_sum = subject_sum + sum(matrix(:, 2));
                subject_count = subject_count + size(matrix, 1);
            end
        end
        subject_means(i) = subject_sum; % Or subject_sum / subject_count if you want the mean reward per trial
        total_sum = total_sum + subject_sum;
        total_count = total_count + subject_count;
    end

    mean_reward = total_sum / numel(data);
end

% Helper function to calculate standard error of reward from the nested cell structure
function se = calculate_standard_error_reward(data)
    subject_means = zeros(1, numel(data));
    ...
    for i = 1:numel(data)
        ...
        subject_means(i) = subject_sum;
    end

    % Calculate standard error using the standard deviation of the subject means
    se = std(subject_means) / sqrt(numel(subject_means));
end


%previous incorrect function
% Helper function to calculate standard error of reward from the nested cell structure
% function se = calculate_standard_error_reward(data)
%     means = [];
%     for i = 1:numel(data)
...
%             matrix = data{i}{j};
%             if ~isempty(matrix)
%                 means(end+1) = mean(matrix(:, 2)); %#ok<AGROW>
%             end
%         end
%     end
%     se = std(means) / sqrt(numel(means));
% end
